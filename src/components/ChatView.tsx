...
export default function ChatView({ chat, category, onRefresh, updateChatLocally, isMobile, onBack }: Props) {
  ...
  async function sendMessage() {
    ...
    try {
      ...
      const res = await llm.chat(chat.model, conversation, chat.model === 'dall-e-3' ? genSize : undefined);
  ...
  async function generateImageFn() {
    ...
    shouldScrollRef.current = true;
    try {
      const aiMsg = await generate.image(genPrompt.trim(), chat.id, genSize);
        ...
        <div className="max-w-3xl mx-auto">
          ...
          {genMode && (
            <div className="mb-2 space-y-2">
              ...
              <div className="flex items-center gap-1.5">
                <span className="text-xs text-slate-400 dark:text-slate-600">Ratio:</span>
                {(['1024x1024', '1792x1024', '1024x1792'] as const).map((s) => {
                  const label = s === '1024x1024' ? '1:1 □' : s === '1792x1024' ? '16:9 ▬' : '9:16 ▮';
                  return (
                    <button
                      key={s}
          ...
          {chat.model === 'dall-e-3' && (
            <div className="flex items-center gap-1.5 mb-2">
              <span className="text-xs text-slate-400 dark:text-slate-600 mr-1">Ratio:</span>
              {(['1024x1024', '1792x1024', '1024x1792'] as const).map((s) => {
                const label = s === '1024x1024' ? '1:1 □' : s === '1792x1024' ? '16:9 ▬' : '9:16 ▮';
                return (
                  <button
...
function MessageBubble({ message, onHover, onLeave, onDelete, onEdit, onViewImage }: { message: Message; onHover: (id: string) => void; onLeave: () => void; onDelete: (id: string) => void; onEdit: (id: string, content: string) => void; onViewImage: (src: string, alt: string) => void }) {
  ...
  return (
    <div className={`flex gap-3 ${isUser ? 'flex-row-reverse' : ''}`}>
      <div
        className={`hidden md:flex w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${
      ...
      <div
        className={`min-w-0 max-w-full md:max-w-[75%] group relative ${isUser ? 'items-end' : ''} ${editing ? '!min-w-[600px]' : ''}`}
      ...
      >
        ...
        {editing ? (
          <div className={`w-full min-w-[600px] rounded-2xl ${isUser ? 'rounded-br-sm bg-sky-500/20 dark:bg-sky-500/15' : 'rounded-bl-sm bg-slate-200/60 dark:bg-slate-800/60'} px-4 py-3`}>
            <textarea
              ref={textareaRef}
              autoFocus
              value={editDraft}
          ...
          <div className="text-[15px] leading-relaxed whitespace-pre-wrap break-words bg-sky-500/20 dark:bg-sky-500/15 text-sky-900 dark:text-sky-100 rounded-2xl rounded-br-sm px-4 py-3">
            {images.length > 0 && (
